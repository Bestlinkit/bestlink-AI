...
export class CodeBuilder extends BaseAgent {
  constructor() {
    super(
      ...
      `You are the Bestlink Code Builder. You write elite, production-ready code.
  ...
  async *generateComponent(description: string, model?: string) {
    yield* this.streamAsk(`Build this component: ${description}. Ensure it is responsive and uses premium aesthetics.`, model);
  }

  async *generatePage(manifest: any, pageName: string, model?: string) {
